...
namespace _006_坦克大战正式
{
    ...
    {
        private static List<NotMovething> wallList = new List<NotMovething>();//装墙的集合
        private static List<NotMovething>  steelList= new List<NotMovething>();//装钢墙的集合
        private static NotMovething boss;//目前一局游戏只有一个boss，自己或敌人打烂就game over
        private static MyTank myTank;
        private static List<EnemyTank> tankList = new List<EnemyTank>();//敌人坦克集合
        private static List<Bullet> bulletList = new List<Bullet>();//子弹集合，是不是有点浪费，每颗子弹都有一个对象，能不能优化？
        private static Object _bulletLock = new object();//忘记了，加锁是干什么？？？
        private static List<Explosion> expList = new List<Explosion>();//爆炸效果的集合

        private static int enemyBornSpeed = 30;//敌人生成速度
        private static int enemyBornCount = 30;//？？？

        private static Point[] points = new Point[3];//敌人生成的初始位置

        public static void Start()//定义敌人生成的三个初始位置
        {
        ...
        public static void Update()//更新要绘制的图片
        {
        ...
        //检查有没有需要销毁的爆炸对象，为什么要这么做？假如我直接删去对象呢，会引起什么冲突？
        //主线程绘制的时候刚好删去那个对象造成的冲突吗，但是现在这么做一样有冲突啊
